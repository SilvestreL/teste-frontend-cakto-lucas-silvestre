...
import { CheckCircle, QrCode, CreditCard, Clock, Zap } from "lucide-react";
...
const SuccessClientActions = dynamicImport(
  () => import("@/features/checkout/SuccessClientActions"),
  {
    ssr: false,
    loading: () => (
...
function generateMockOrderData(orderId: string) {
  ...
  return {
    ...
    orderData: {
      ...
      createdAt: new Date(Number.parseInt(orderId.split("-")[1])),
      pixCode: isPix
...
export default function SuccessPage({ searchParams }: SuccessPageProps) {
  ...
  const { orderData } = generateMockOrderData(orderId);
      ...
      <Container className="py-8">
        <div className="max-w-2xl mx-auto space-y-8">
          ...
          <div className="text-center space-y-2">
            ...
            <button className="text-brand hover:text-brand-hover text-sm underline">
              suporte@cakto.com
            </button>
          </div>
        </div>
